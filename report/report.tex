...
\title{Saving Hollywood:\\ Can Movie Revenues Be Predicted?}
...
\and
\IEEEauthorblockN{Kashif Javid}
\IEEEauthorblockA{School of Computer Science\\
McGill University\\
260535684\\
Email: kashif.javid@mail.mcgill.ca}
\and
...
\begin{abstract}
%\boldmath
Abstract is necessary, please put abstract. 
\end{abstract}
...
mark. Figures for James Cameron's Avatar Budget reaching up to
...
This presents a problem. It would be important to know the potential 
success of a movie in order to valuate its investment potential. However, 
movies are complex creations involving a large cast and crew. 
On top these factors, amongst others, a movie's success is 
affected by marketing, public perception and critical ratings. This makes
predicting the potential revenue for a planned movie a challenging 
task. We describe this task as follows: 

\textit{\textbf{Produce a predictor using some constructed feature set that allows us 
to accurately predict the gross revenue of a movie.}}

In this document we will describe our attempt at tackling this task through
several linear regression models using our feature set constructed from parsing
...
by critics and audiences for the directors, studios, screen writers and main actors of the movie. Lastly, 
we have the budget of the movie and the target value, gross revenue for a movie. The dictionaries for 
...
varying parameters in order to draw insights of our data. 
...
The first is a collection movie information with budgets, revenues, cast, crew with all movies 
indexed but has incomplete information for a large number of movies. The second one is a large site and 
community but has no simple movie index to parse data through. 

We use the first to extract a list of movies ~20,000 and 
...
This gives us ~4,000 movies. We then use the movie names 
...
repeat this for the directors, producers and screenwriters of the movie. We produce 
a rating per person for the "Critical" Rating from Rotten Tomatoes, the "Audience" 
...
We apply the typical gradient descent using a step size of ------??--------- and 
max iterations of ---------??------------. We repeat the 10-fold validation and the 
k-variation from linear regression for gradient descent. We also vary the step size
alpha from ===============????=============== to ============????=======
and similarly range iterations from ===========????-------- to =========??========.
...
Since we rely on the average ratings for the cast on crew over all their movies to generate
the measure for their respective set on the movies, we need to take care in creating the mappings
that gives us the average rating for a given person. That is, we regenerate the feature set for each
k fold validation in order to not pollute the input. In the case that an actor, director, producer or 
screen writer is not present in a mapping, we simply assing them a 50\% rating for rotten tomato and
5.0 for imdb. That is we do not assume any information for missing cases. 

% add how we messed with different parameters and stuff here 
